...
static void epf_vnet_qnotify_callback(void *param)
{
	...
	queue_work(vnet->task_rx_wq, &vnet->ep_ctrl_work);
}
...
static void epf_vnet_init_complete(struct epf_vnet *vnet, u8 from)
{
	...
	spin_lock_irqsave(&vnet->init_state_lock, flags);
	vnet->init_state |= from;
	spin_unlock_irqrestore(&vnet->init_state_lock, flags);

	if (!(vnet->init_state & EPF_VNET_INIT_COMPLETE_VDEV))
		return;

	if (!(vnet->init_state & EPF_VNET_INIT_COMPLETE_EP_FUNC))
		return;
...
static void epf_vnet_complete_xfer(struct epf_vnet *vnet,
				   unsigned int local_vq_index,
				   unsigned int remote_vq_index,
				   u16 remote_head, u16 local_head,
				   size_t total_len)
{
	struct epf_virtio *evio = &vnet->evio;
	...
	epf_virtio_iov_complete(evio, remote_vq_index, remote_head, total_len);
...
static void epf_vnet_raise_irq_handler(struct work_struct *work)
{
	...
	pci_epc_raise_irq(epf->epc, epf->func_no, epf->vfunc_no, PCI_IRQ_INTX,
			  0);
}
