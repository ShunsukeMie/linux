...
{
	return vringh_iov_xfer(NULL, riov, dst, len, vrh->ops.xfer_from);
}
...
{
	return vringh_iov_xfer(NULL, wiov, (void *)src, len, vrh->ops.xfer_to);
}
...
static int _vringh_init_iotlb(struct vringh *vrh, u64 features,
		      unsigned int num, bool weak_barriers, gfp_t gfp,
		      struct vring_desc *desc,
		      struct vring_avail *avail,
		      struct vring_used *used)
		      ...
		      struct vring_used *used)
{
	...
	return _vringh_init_iotlb(vrh, features, num, weak_barriers, gfp, desc, avail, used);
}
